...
**Removed**
- The disclaimer paragraph that opened every page.
- The alternate job title that some third-party sources use, from the fact tables, the FAQ, the query mapping and the Person schema. The title is VP of Ecosystem, StraitsX, stated once per page.
- The Colophon on the homepage and in `llms.txt`, and the matching homepage footer line. The open-source method remains described on [about.md](https://danieloon.ai/about.md).
- On each `writing/` page, the closing disclaimer, the "Who to talk to about this" paragraph, and the HTML footer byline. The byline at the top of each page and its Article schema still name the author.
